...
// Helper: online URL or local file
function getHotelImg($imgField, $prefix = '../Hotel/') {
    $imgs = explode(',', $imgField);
    $first = trim($imgs[0]);
    if (empty($first)) return '../img/logo.png';
...
<head>
  ...
  <style>
     ...
     .hotel-card { background: white; border-radius: 20px; overflow: hidden; border: 1px solid #e2e8f0; transition: all 0.3s ease; height: 100%; display: flex; flex-direction: column; box-shadow: 0 4px 15px rgba(0,0,0,0.02); }
     .hotel-card:hover { transform: translateY(-8px); box-shadow: 0 12px 25px rgba(0,0,0,0.08); }
     ...
     .card-actions { padding: 15px 20px; background: #f8fafc; display: flex; gap: 10px; border-top: 1px solid #f1f5f9; }
     .btn-action { flex: 1; padding: 10px; border-radius: 25px; font-size: 0.85rem; font-weight: 500; text-align: center; text-decoration: none; transition: all 0.3s ease; }
     ...
     .btn-create { background: #0fb9b1; color: white; border: none; padding: 10px 25px; border-radius: 12px; font-weight: 500; box-shadow: 0 4px 12px rgba(15,185,177,0.2); text-decoration: none; }
