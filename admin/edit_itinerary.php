...
if (!isset($_GET['id'])) {
    echo "<script>alert('Invalid Request'); window.location='view_itinerary.php';</script>";
    exit;
}
...
$result = mysqli_query($conn, "SELECT * FROM itineraries WHERE id=$id");
if (mysqli_num_rows($result) == 0) {
    echo "<script>alert('Itinerary not found'); window.location='view_itinerary.php';</script>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE itineraries SET package_id=?, day_or_night=?, plan=? WHERE id=?");
    $stmt->bind_param("issi", $package_id, $day_or_night, $plan, $id);

    if ($stmt->execute()) {
        // Redirect back with success flag
        header("Location: edit_itinerary.php?id=$id&updated=1");
        exit;
    } else {
...
<div class="main">
    ...
    <div class="form-container col-lg-8 mx-auto">
        <h2 class="form-title text-center"><i class="bi bi-pencil-square me-2 text-primary"></i> Edit Day Plan</h2>
        ...
        <form method="POST">
            <div class="mb-4">
                ...
                <select name="package_id" class="form-select" required>
                    ...
                    <?php while($pkg = mysqli_fetch_assoc($packages)) { ?>
                        <option value="<?= $pkg['id'] ?>" <?= $pkg['id']==$itinerary['package_id']?'selected':'' ?>>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="day_or_night" class="form-control" required
                       value="<?= htmlspecialchars($itinerary['day_or_night']) ?>">
            </div>

            <div class="mb-4">
                <label class="form-label">Detailed Plan</label>
                <textarea name="plan" class="form-control" rows="8" required><?= htmlspecialchars($itinerary['plan']) ?></textarea>
            </div>

            <button type="submit" class="btn-submit">
                <i class="bi bi-save me-2"></i> Update Itinerary Day
            </button>
        </form>
    </div>
</div>
