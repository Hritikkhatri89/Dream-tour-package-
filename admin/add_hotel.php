...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_query($conn, $sql)) {
        echo "<script>alert('✅ Hotel added successfully!'); window.location.href='add_hotel.php';</script>";
    } else {
...
<style>
 body { font-family: 'Outfit', sans-serif; background: #f4f7fa; margin: 0; color: #1e293b; overflow-x: hidden; }

 .sidebar { height: 100vh; width: 260px; position: fixed; top: 0; left: 0; background: #0F172A; padding: 20px 15px; color: white; box-shadow: 4px 0 20px rgba(0,0,0,0.1); z-index: 1000; overflow-y: auto; }
 .sidebar h4 { font-weight: 700; margin-bottom: 25px; text-align: center; text-transform: uppercase; letter-spacing: 2px; color: #0fb9b1; font-size: 1.4rem; }
 .sidebar a { display: flex; align-items: center; padding: 10px 15px; margin-bottom: 10px; color: #94a3b8; border-radius: 12px; text-decoration: none; transition: all 0.3s ease; font-weight: 500; font-size: 0.85rem; }
 .sidebar a i { margin-right: 15px; font-size: 1.2rem; }
 .sidebar a:hover, .sidebar a.active { background: rgba(255,255,255,0.05); color: white; transform: translateX(5px); }
 ...
 .form-container { background: white; padding: 40px; border-radius: 24px; box-shadow: 0 10px 30px rgba(0,0,0,0.03); border: 1px solid #e2e8f0; }
 ...
 .form-control, .form-select { padding: 12px 18px; border-radius: 12px; border: 1px solid #e2e8f0; background: #f8fafc; color: #1e293b; font-size: 0.85rem; transition: all 0.3s ease; }
 .form-control:focus, .form-select:focus { background: #fff; border-color: #0fb9b1; box-shadow: 0 0 0 4px rgba(15,185,177,0.1); }
 .btn-submit { background: #0fb9b1; color: white; border: none; padding: 14px 40px; border-radius: 12px; font-weight: 500; font-size: 1rem; transition: all 0.3s ease; box-shadow: 0 4px 12px rgba(15,185,177,0.2); }
 .btn-submit:hover { background: #0da59e; transform: translateY(-2px); }
 .preview-container { display: flex; flex-wrap: wrap; gap: 15px; margin-top: 15px; }
 .preview-container img { width: 120px; height: 90px; object-fit: cover; border-radius: 12px; border: 2px solid #f1f5f9; }
 ...
 .btn-back { background: #fff; color: #64748b; border: 1px solid #e2e8f0; border-radius: 12px; padding: 8px 20px; text-decoration: none; display: inline-flex; align-items: center; margin-bottom: 25px; font-weight: 500; transition: all 0.3s ease; }
...
<!-- Main Content -->
<div class="main">
    <a href="view_hotel.php" class="btn-back"><i class="bi bi-arrow-left me-2"></i> Back to Hotels</a>
    ...
    <div class="form-container col-lg-10 mx-auto">
        ...
        <form method="POST" enctype="multipart/form-data">
            <div class="row g-4">
                ...
                <div class="col-md-12">
                    ...
                    <select name="package_id" class="form-select" required>
                        ...
                        while($pkg = mysqli_fetch_assoc($packages)): ?>
                            <option value="<?= $pkg['id']; ?>" <?= ($selected_pkg_id == $pkg['id']) ? 'selected' : '' ?>><?= htmlspecialchars($pkg['title']); ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                ...
                <div class="col-md-12">
                    <label class="form-label">UPLOAD GALLERY IMAGES <small class="text-muted fw-normal">(Optional)</small></label>
